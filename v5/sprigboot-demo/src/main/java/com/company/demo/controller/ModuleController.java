...
//    @UserLoginToken
    ...
    public Result addModule(@RequestBody Module module) {
        if (module.getName() == null || module.getName().equals("")) {
            return ResultUtil.error(PARAMS_ERROR_CODE, "模块名称必填");
        }
        if (module.getUrl() == null || module.getUrl().equals("")) {
            return ResultUtil.error(PARAMS_ERROR_CODE, "模块url必填");
        }
        //名称重复判断
        Module m = new Module();
        ...
        if (rm.size() > 0) {
            return ResultUtil.error(PARAMS_ERROR_CODE, "模块名称不能重复");
        }
    ...
    public Result updateModule(@RequestBody Module module){
        if(module.getId()==null||module.getId().equals("")){
            return ResultUtil.error(PARAMS_ERROR_CODE,"id不存在");
        }
        if (module.getName() == null || module.getName().equals("")) {
            return ResultUtil.error(PARAMS_ERROR_CODE, "模块名称必填");
        }
